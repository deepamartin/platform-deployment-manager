        ...
        try:
            ...
            if spark_jobs:
                ...
                information = {
                    ...
                    'complete': stage_complete_c, 'pending': stage_pending_c, 'failed': stage_failed_c}
                }
                if job_fail_c >= 1:
                    ...
                    if action['externalChildIDs']:
                        ...
                        actual_job['yarnFinalStatus'] == 'FAILED':
                            ...
                            if spark_job['state'] == 'ERROR':
                                status = COMPONENT_STATUS['red']
                                information = {'sparkJob': spark_job['information']}
                            else:
                                status = COMPONENT_STATUS['green']
                                information = {'sparkJob': spark_job['information']}
                        else:
...
def oozie_coordinator_handler(data):
    ...
    if data['status'] == 'PREPSUSPENDED':
        coord_status.update({'aggregate_status': ApplicationState.CREATED, 'name': data['coordJobName']})
    elif data['status'] == 'PREP':
        coord_status.update({'aggregate_status': ApplicationState.STARTING, 'name': data['coordJobName']})
...
def oozie_workflow_handler(data):
    ...
    workflow_status = {}
    if data['status'] == 'PREP':
        workflow_status.update({'aggregate_status': ApplicationState.CREATED, 'name': data['appName']})
    elif data['status'] == 'RUNNING':
        oozie_data = get_oozie_workflow_actions(data['actions'])
        aggregate_status = process_component_data(oozie_data)
    ...
    try:
        ...
        try:
            ...
            if yarn_list['apps'] != None:
                for main_ele in yarn_list:
                    for sub_ele in yarn_list[main_ele]:
                        for ele in yarn_list[main_ele][sub_ele]:
                            if job_name == ele['name']:
                                if run_app_info is None or compare_yarn_start_time(ele) > \
